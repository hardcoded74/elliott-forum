...
console.warn("Can not detect viewport width.");return a};d.prototype.replace=function(a){this.$stage.empty();this._items=[];a&&(a=a instanceof jQuery?a:c(a));this.settings.nestedItemSelector&&(a=a.find("."+this.settings.nestedItemSelector));a.filter(function(){return 1===this.nodeType}).each(c.proxy(function(a,c){c=this.prepare(c);this.$stage.append(c);this._items.push(c);this._mergers.push(1*c.find("[data-merge]").addBack("[data-merge]").attr("data-merge")||1)},this));this.reset(this.isNumeric(this.settings.startPosition)?
this.settings.startPosition:0);this.invalidate("items")};d.prototype.add=function(a,b){var e=this.relative(this._current);b=b===k?this._items.length:this.normalize(b,!0);a=a instanceof jQuery?a:c(a);this.trigger("add",{content:a,position:b});a=this.prepare(a);0===this._items.length||b===this._items.length?(0===this._items.length&&this.$stage.append(a),0!==this._items.length&&this._items[b-1].after(a),this._items.push(a),this._mergers.push(1*a.find("[data-merge]").addBack("[data-merge]").attr("data-merge")||
...
{content:null,position:a}))};d.prototype.preloadAutoWidthImages=function(a){a.each(c.proxy(function(a,e){this.enter("pre-loading");e=c(e);c(new Image).one("load",c.proxy(function(a){e.attr("src",a.target.src);e.css("opacity",1);this.leave("pre-loading");this.is("pre-loading")||this.is("initializing")||this.refresh()},this)).attr("src",e.attr("src")||e.attr("data-src")||e.attr("data-src-retina"))},this))};d.prototype.destroy=function(){this.$element.off(".owl.core");this.$stage.off(".owl.core");c(l).off(".owl.core");
...
d.attr("data-src")||d.attr("data-srcset");this._core.trigger("load",{element:d,url:e},"lazy");if(d.is("img"))d.one("load.owl.lazy",c.proxy(function(){d.css("opacity",1);this._core.trigger("loaded",{element:d,url:e},"lazy")},this)).attr("src",e);else if(d.is("source"))d.one("load.owl.lazy",c.proxy(function(){this._core.trigger("loaded",{element:d,url:e},"lazy")},this)).attr("srcset",e);else{var m=new Image;m.onload=c.proxy(function(){d.css({"background-image":'url("'+e+'")',opacity:"1"});this._core.trigger("loaded",
{element:d,url:e},"lazy")},this);m.src=e}},this)),this._loaded.push(a.get(0)))};d.prototype.destroy=function(){var a,b;for(a in this.handlers)this._core.$element.off(a,this.handlers[a]);for(b in Object.getOwnPropertyNames(this))"function"!=typeof this[b]&&(this[b]=null)};c.fn.owlCarousel.Constructor.Plugins.Lazy=d})(window.Zepto||window.jQuery,window,document);
(function(c,f,l,k){var d=function(a){this._core=a;this._handlers={"initialized.owl.carousel refreshed.owl.carousel":c.proxy(function(a){a.namespace&&this._core.settings.autoHeight&&this.update()},this),"changed.owl.carousel":c.proxy(function(a){a.namespace&&this._core.settings.autoHeight&&"position"===a.property.name&&(console.log("update called"),this.update())},this),"loaded.owl.lazy":c.proxy(function(a){a.namespace&&this._core.settings.autoHeight&&a.element.closest("."+this._core.settings.itemClass).index()===
...
b=a+this._core.settings.items;a=this._core.$stage.children().toArray().slice(a,b);var d=[];b=0;c.each(a,function(a,b){d.push(c(b).height())});b=Math.max.apply(null,d);this._core.$stage.parent().height(b).addClass(this._core.settings.autoHeightClass)};d.prototype.destroy=function(){var a,b;for(a in this._handlers)this._core.$element.off(a,this._handlers[a]);for(b in Object.getOwnPropertyNames(this))"function"!==typeof this[b]&&(this[b]=null)};c.fn.owlCarousel.Constructor.Plugins.AutoHeight=d})(window.Zepto||
window.jQuery,window,document);
(function(c,f,l,k){var d=function(a){this._core=a;this._videos={};this._playing=null;this._handlers={"initialized.owl.carousel":c.proxy(function(a){a.namespace&&this._core.register({type:"state",name:"playing",tags:["interacting"]})},this),"resize.owl.carousel":c.proxy(function(a){a.namespace&&this._core.settings.video&&this.isInFullScreen()&&a.preventDefault()},this),"refreshed.owl.carousel":c.proxy(function(a){a.namespace&&this._core.is("resizing")&&this._core.$stage.find(".cloned .owl-video-frame").remove()},this),
"changed.owl.carousel":c.proxy(function(a){a.namespace&&"position"===a.property.name&&this._playing&&this.stop()},this),"prepared.owl.carousel":c.proxy(function(a){if(a.namespace){var b=c(a.content).find(".owl-video");b.length&&(b.css("display","none"),this.fetch(b,c(a.content)))}},this)};this._core.options=c.extend({},d.Defaults,this._core.options);this._core.$element.on(this._handlers);this._core.$element.on("click.owl.video",".owl-video-play-icon",c.proxy(function(a){this.play(a)},this))};d.Defaults=
{video:!1,videoHeight:!1,videoWidth:!1};d.prototype.fetch=function(a,b){a.attr("data-vimeo-id")||a.attr("data-vzaar-id");a.attr("data-vimeo-id")||a.attr("data-youtube-id")||a.attr("data-vzaar-id");var c=a.attr("data-width")||this._core.settings.videoWidth,d=a.attr("data-height")||this._core.settings.videoHeight,h=a.attr("href");if(h){var g=h.match(/(http:|https:|)\/\/(player.|www.|app.)?(vimeo\.com|youtu(be\.com|\.be|be\.googleapis\.com)|vzaar\.com)\/(video\/|videos\/|embed\/|channels\/.+\/|groups\/.+\/|watch\?v=|v\/)?([A-Za-z0-9._%-]*)(&\S+)?/);
...
c("<div>").addClass(b.dotsClass).appendTo(this.$element)).addClass("disabled");this._controls.$absolute.on("click","button",c.proxy(function(a){var d=c(a.target).parent().is(this._controls.$absolute)?c(a.target).index():c(a.target).parent().index();a.preventDefault();this.to(d,b.dotsSpeed)},this));for(a in this._overrides)this._core[a]=c.proxy(this[a],this)};d.prototype.destroy=function(){var a,b,c,d,f=this._core.settings;for(a in this._handlers)this.$element.off(a,this._handlers[a]);for(b in this._controls)"$relative"===
b&&f.navContainer?this._controls[b].html(""):this._controls[b].remove();for(d in this.overides)this._core[d]=this._overrides[d];for(c in Object.getOwnPropertyNames(this))"function"!=typeof this[c]&&(this[c]=null)};d.prototype.update=function(){var a,b,c=this._core.clones().length/2,d=c+this._core.items().length,f=this._core.maximum(!0),g=this._core.settings,k=g.center||g.autoWidth||g.dotsData?1:g.dotsEach||g.items;"page"!==g.slideBy&&(g.slideBy=Math.min(g.slideBy,g.items));if(g.dots||"page"==g.slideBy)for(this._pages=
[],g=c,b=a=0;g<d;g++){if(a>=k||0===a){this._pages.push({start:Math.min(f,g-c),end:g-c+k-1});if(Math.min(f,g-c)===f)break;a=0;++b}a+=this._core.mergers(this._core.relative(g))}};d.prototype.draw=function(){var a=this._core.settings,b=this._core.items().length<=a.items,d=this._core.relative(this._core.current()),f=a.loop||a.rewind;this._controls.$relative.toggleClass("disabled",!a.nav||b);a.nav&&(this._controls.$previous.toggleClass("disabled",!f&&d<=this._core.minimum(!0)),this._controls.$next.toggleClass("disabled",
!f&&d>=this._core.maximum(!0)));this._controls.$absolute.toggleClass("disabled",!a.dots||b);a.dots&&(b=this._pages.length-this._controls.$absolute.children().length,a.dotsData&&0!==b?this._controls.$absolute.html(this._templates.join("")):0<b?this._controls.$absolute.append(Array(b+1).join(this._templates[0])):0>b&&this._controls.$absolute.children().slice(b).remove(),this._controls.$absolute.find(".active").removeClass("active"),this._controls.$absolute.children().eq(c.inArray(this.current(),this._pages)).addClass("active"))};
d.prototype.onTrigger=function(a){var b=this._core.settings;a.page={index:c.inArray(this.current(),this._pages),count:this._pages.length,size:b&&(b.center||b.autoWidth||b.dotsData?1:b.dotsEach||b.items)}};d.prototype.current=function(){var a=this._core.relative(this._core.current());return c.grep(this._pages,c.proxy(function(b,c){return b.start<=a&&b.end>=a},this)).pop()};d.prototype.getPosition=function(a){var b=this._core.settings;if("page"==b.slideBy){var d=c.inArray(this.current(),this._pages);
b=this._pages.length;a?++d:--d;d=this._pages[(d%b+b)%b].start}else d=this._core.relative(this._core.current()),this._core.items(),a?d+=b.slideBy:d-=b.slideBy;return d};d.prototype.next=function(a){c.proxy(this._overrides.to,this._core)(this.getPosition(!0),a)};d.prototype.prev=function(a){c.proxy(this._overrides.to,this._core)(this.getPosition(!1),a)};d.prototype.to=function(a,b,d){!d&&this._pages.length?(d=this._pages.length,c.proxy(this._overrides.to,this._core)(this._pages[(a%d+d)%d].start,b)):
c.proxy(this._overrides.to,this._core)(a,b)};c.fn.owlCarousel.Constructor.Plugins.Navigation=d})(window.Zepto||window.jQuery,window,document);
